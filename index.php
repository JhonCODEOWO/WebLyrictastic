<?php
    ...
    $query = "SELECT letras.id AS id_letra, usuario.id AS id_usuario, letras.*, usuario.* FROM letras inner join usuario on usuario.id = letras.usuario_id ORDER BY  likes DESC LIMIT 5;";
                ...
                <div class="letra-tarjeta overlay-tarjeta">
                    <div class="letra-tarjeta-content-blur">
                        <a class="without-transition-a" href="lyric.php?id=<?php echo $letra['id_letra'] ?>&s=index">
                            ...
                            <h3 class="light"> <?php echo $letra['nombre'] ?></h3>
                        </div>
                        </a>
                    </div>
                </div>
